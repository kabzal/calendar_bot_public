...
cal: InlineKeyboardButton = InlineKeyboardButton(text='Мой календарь', callback_data='show_calendar')
my_clients: InlineKeyboardButton = InlineKeyboardButton(text='Мои записи', callback_data='my_clients')
...
async def kb_all_times(all_times: list, current_date):
    ...
    for time in all_times:
        if time[3] == 'free':
            b_all_times.append(InlineKeyboardButton(text=(f'{time[1]} - 🟢'), callback_data=(f'all_times {time[1]} {current_date} {time[3]}')))
        elif time[3] == 'appointment':
            b_all_times.append(InlineKeyboardButton(text=(f'{time[1]} - 🔴'), callback_data=(f'all_times {time[1]} {current_date} {time[3]}')))
    b_all_times.append(InlineKeyboardButton(text='Добавить', callback_data='add_new_appoint'))
    b_all_times.append(InlineKeyboardButton(text='Вернуться', callback_data='show_calendar'))
...
async def kb_ok_cancel_func(current_date, current_time):
    okey: InlineKeyboardButton = InlineKeyboardButton(text='Назад к списку', callback_data='my_clients')
